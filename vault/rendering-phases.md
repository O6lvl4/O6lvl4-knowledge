...
updated_at: 2026-07-03T08:00:00+09:00
---
...
2 つの軸は組み合わせ自由。Next.js は「SSR/SSG + SPA」、Rails は「SSR + MPA」、Vite + React の素の構成は「CSR + SPA」。「SPA = CSR」と覚えると混乱するので、**質問が違う 2 つの軸**として覚える。
...
CSR とセットで語られることが多いが、**軸が違う**。CSR/SSR/SSG が「初回アクセス」の話なのに対し、SPA は**初回の後、ページを移動するとき**の話（冒頭の「まず地図」の 2 軸目）。
...
- 4 つの言葉の地図 → CSR/SSR/SSG は「初回の HTML をいつ作るか」、SPA/MPA は「2 ページ目以降をどう動かすか」。質問が違う 2 軸で、組み合わせは自由
