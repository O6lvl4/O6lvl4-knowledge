...
ただし本ノートは**設計段階の記録**であり、運用実績に裏打ちされていない。[[retrieval-practice|理論側]]が実証研究に支えられているのに対し、こちらは理論から演繹した構成にすぎない。閾値や規則の妥当性は、回してみるまで分からない。
...
**理論が関数形を与えていないから。** Bjork の2軸モデルは「貯蔵強度の増分は引き出し強度の減少関数」としか言わず、その関数の形を定めない。自作すると、理論が空けている穴を勘の定数で埋めることになる。
...
S の定義の仕方に注目したい。貯蔵強度は直接測れない構成概念だが、[[retrieval-practice|規則3]](貯蔵強度が高いほど減衰が遅い)を逆に使えば、**減衰の遅さそのものを貯蔵強度の定義にできる**。測れない量を、測れる帰結で置き換える ── 構成概念を工学量に変換する典型的な手口で、L1 が既に解かれている理由でもある。

したがって L1 に手を出すなら、それは **FSRS の再実装という別プロジェクト**になる。学習の道具作りとは切り離して扱う。混ぜると、学習が進まないまま実装だけが残る。
...
`運用中 → 要還流 → 理解中` の戻り辺があるため、直線ではなくループになる。この辺のおかげで、カードの失敗が薄いノートを名指しで教えてくれる。戻り辺を欠くと、落ち続けるカードを永久に回すだけの装置になる。
...
「習得モード」と「本番前モード」はシステム全体にかかるので、個々の項目の状態とは別に持ち、状態機械と直交させる。
...
本番前モードは、わざとミスさせる設計をヒットさせ続ける設計に反転させる操作で、スケジューラの目的関数を意図的に裏切る。だから状態として持つと破綻し、モードとして外から被せる必要がある。学習効率は落ちるが、それを承知でやる期間だと自覚できていれば正しい判断になる。
