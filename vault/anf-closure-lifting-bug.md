...
- [[almide-differential-gate|差分ゲート]] — 同じ「事後検証が本物のバグを検出する」思想の実行時版(postcondition に対する出力照合)
- [[traversal-totality|走査網羅性]] — このバグの**源流クラス**。ANF が `__closure_*` を走査し損ねたのは「サイト取りこぼし」の一事例
