...
- ペトリネットとは → 並行システムの状態遷移を数学的にモデル化する手法 (Petri, 1962)。デッドロックや到達不能状態の有無を検証できる
- 3要素＋マーキング → Place (場所＝条件/フラグ)、Transition (遷移＝イベント)、Token (トークン＝フラグの数)。Marking が現在の状態 (各場所のトークン数)
- 発火規則 → 遷移は入力場所すべてに必要数のトークンがあれば enabled。発火すると入力からトークンを消費し出力に生成する
- HTN Planning との違い → HTN は「ゴールへの手順」を前向きに計画 (何をするか決める)、ペトリネットは全状態遷移をモデル化して性質を検証 (問題が起きないか確かめる)。併用が強い
- 検証できる4性質 → 到達可能性 (ある状態に到達できるか)、有界性 (トークンが無限増殖しないか)、活性 (どの遷移もいつか発火可能＝デッドロック検出)、安全性 (各場所のトークンが1以下か)
