...
- [[obs-studio-architecture]] — 内部アーキテクチャ分析（4層構造、データフロー、部分実装の独立性）
- [[obs-libobs]] — libobs コアライブラリの詳細（サブシステム、Source の内部分類）
- [[obs-plugin-system]] — プラグインシステムの全体像（38プラグインの分類）
- [[streaming-software|配信ソフトウェア]] — このジャンルの抽象概念（プロトコル・構成要素）
- [[3tene]] — OBS に映像を渡す VTuber アプリ
- [[face-tracking|フェイストラッキング]] — アバター駆動の入力元
