...
4つとも「今の出来」と「本当に入った量」がズレることで起きる。1つの量ではこのズレを表現できないため、軸を2本に割る必要があった。Bjork の言い方では **performance(今の出来) ≠ learning(習得)**。
...
分散学習(spacing)が効くのは、忘れかけを作れるため。この枠組みで見ると、分散は独立した技法というより**規則2と3から出る最適化問題の解**にあたり、「引き出し強度が下がりきる直前を狙え」がその解の形になる。
...
**流暢性の錯覚**(fluency illusion)。手応えと効果が逆相関するため、体感で方法を選ぶと構造的に必ず間違ったほうを選ぶ。したがって学習法は原則で選ぶしかない。「読み返すな、テストしろ」に根拠があるのはこのため。
...
言い換えとは、元の表現という手がかりを外した状態で、意味だけを頼りに再構成する作業 — **想起の一種**にあたる。だから効く。転記が効かないのは、手がかりを外していないため。差は勤勉さではなく原理の側にある。
...
一方で**手書き vs タイプ**の軸は弱い。よく引かれる Mueller & Oppenheimer (2014) は Morehead, Dunlosky & Rawson (2019) の直接再現で再現せず、直接再現のメタ分析でも手書き有利は小さく非有意だった。効いていたのは筆記具ではなく、手書きだと転記が物理的に不可能で要約せざるを得ない点にある。同じ機序はデジタルでも再現でき、しかも分散学習の管理はソフトでしか実行できない。
