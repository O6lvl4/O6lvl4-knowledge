...
**VRoid Studio の競合アプリを作って公開すること。** 具体的には、VRoid Studio で作ったメッシュやテクスチャを素材として組み込んで「ユーザーが 3D モデルを生成・出力できるアプリやサービス」を公開することが禁止されている。要は VRoid のアセットを流用した別のキャラクタークリエイターを作って配るな、という話。

VTuber 配信、ゲームに組み込む、グッズにする、VRM を販売する — こういった「モデルを使う」行為はすべて OK。自分だけが使う内製ツールもこの制限の対象外。
