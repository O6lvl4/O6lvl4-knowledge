...
| VRM の中身 | 権利者 | 変わる？ | 根拠 |
|---|---|---|---|
| パラメータの組み合わせ（顔、体型、構成） | **あなた** | 定数 | 利用規約 第9条 |
| 自分で描いたテクスチャ | **あなた** | 定数 | 著作権法（作った瞬間に発生） |
| プリセット髪型・衣装のメッシュ | **pixiv** | 変数 | ガイドラインで許諾（現時点は商用OK） |
| デフォルトテクスチャ（肌、目など） | **pixiv** | 変数 | 同上 |
| 標準ボーン構造 | **pixiv** | 変数 | 同上 |
| BOOTH で買った素材 | **その作者** | 変数 | 素材ごとのライセンス |
