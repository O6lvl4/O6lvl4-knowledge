...
- [[famulus2]] — MCP プロトコル対応モジュールを内蔵し、外部ツールとの相互運用を実現
- [[claude-code]] — MCP クライアントとして外部 MCP サーバを接続できる
- [[porta]] — WASM エージェントを MCP サーバとして公開する「セキュア MCP ブリッジ」
- [[obsid]] — 別系統の WASM ホスト
- [[unillm]] — LLM プロバイダの統一 API。MCP と相補的（モデル側の抽象）
- [[sandboxes-o6lvl4]] — 隔離環境上で MCP サーバを動かす流儀

## 関連概念

- [[capability-based-security]] — MCP サーバへの権限を細かい鍵で渡す
- [[oauth]] — MCP サーバが外部 API を叩く際の認可手順
