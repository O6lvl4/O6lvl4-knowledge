...
updated_at: 2026-06-01T16:42:37+09:00
---
...
> 以下の記述は **commit `779944d`**(2026-05-04, PR #203 merge)時点の clone に基づく。API は experimental で変わりうるため、版を固定して確認した事実を残す。
...
```
[JS shim] worker.mjs + wasm_exec_go.js / wasm_exec_tinygo.js
  - workers-assets-gen が生成(runtime/cloudflare.mjs 等)
  - wasm を instantiate し、fetch/scheduled/queue を wasm の export に配線
        ↕ syscall/js (js.Value)
[Go (wasm)]
...
JS 資産(wasm_exec + worker.mjs)は付属ツール **`workers-assets-gen`** が吐く。`go.mod` は **`go 1.21.3`**。パッケージは core の `workers`(`Serve` / `ServeNonBlock`)と `cloudflare/{kv,r2,d1,cache,cron,queues,sockets,fetch}` サブパッケージに分かれる。
...
| バインディング | Go API(`cloudflare/*`) |
|---|---|
| **KV** | `kv.NewNamespace("VAR")` → `GetString` / `GetReader` / `PutString` / `PutReader` / `List` / `Delete` |
| **R2** | `Head` / `Get` / `Put` / `Delete` / `List` |
| **Cache API** | 対応 |
| **Durable Objects** | stub 呼び出し |
| **D1** | **`database/sql` ドライバとして実装**(`init` でドライバ登録 → `sql.Open("d1", …)` で使える) |
| **Cron** | `cron.ScheduleTask(task)` / `ScheduleTaskNonBlock` |
| 環境変数 | `cloudflare.Getenv` 等 |

```go
// KV: 標準的な Go API として叩ける(内部は syscall/js)
ns, _ := kv.NewNamespace("MY_KV")
_ = ns.PutString("k", "v", nil)
v, _ := ns.GetString("k", nil)
```

D1 が `database/sql` ドライバなのが効いていて、**既存の Go の SQL コード(`sql.Open`/`Query`/`Exec`)がそのまま乗る** — net/http がそのまま乗るのと同じ「既存資産の再利用」思想。
...
- [syumai/workers (GitHub)](https://github.com/syumai/workers) — 確認版 commit `779944db4752699bf9ff7386cd34d24ce3d5870d`(2026-05-04)
