...
│                                      │ steal      │
│                      ←───────────────┘ (FIFO)    │
│                 Thread 0 の Deque 底から盗む      │
└─────────────────────────────────────────────────┘
```

各ワーカースレッドが**ローカルの deque (両端キュー)** を持つ。
...
```
join(task_a, task_b)
  ├── task_a を現スレッドで即座に実行開始
  └── task_b を deque に push（他スレッドが盗める状態にする）
```
...
- **まだ盗まれていない** → 自分の deque から pop して実行
- **既に盗まれて実行中** → 完了を待つ間、**他スレッドの仕事を steal して処理** (idle waiting ではない)

この「待ちながらも steal する」動作が、スレッドを遊ばせない設計の核心。

## 分割統治との相性

work-stealing は**再帰的な分割統治 (divide-and-conquer)** と最も相性が良い：
