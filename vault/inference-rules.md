...
- 推論規則とは → 横線の上が前提（成り立つなら）、下が結論（導ける）という形式記法。型システムの型付け規則はこれで定義される
- 基本ルール → Var(スコープの x:T から x:T)、App(f:A→B と a:A から f(a):B)、Abs(x:A 仮定で body:B なら関数は A→B)
- 制御構文のルール → If は両ブランチが同じ型 T で結果 T、Let は e1:A の下で e2:B なら全体 B
- 型推論 = 逆向き適用 → 型を書かなくても推論規則から型を導出する。+ が number×number→number といった組み込みルールを使って合成
- Hindley-Milner → アノテーションなしでも最も一般的な型を自動推論する方式(Haskell, OCaml)
